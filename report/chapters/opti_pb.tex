...
\end{equation}
where $T_0$ is the solution of the problem \emph{without source} ($f=0$) with the same boundary conditions (Dirichlet and Robin), and for $i=1,\ldots,6$, $T_i$ is the solution of the homogeneous Dirichlet temperature problem ($T_i|_{\Gamma_{\text{top}}}=0$) with the right-hand side $f = \mathbf{1}_{C_i}$:
\begin{equation}
-\mathrm{div}(k\nabla T_i) = \mathbf{1}_{C_i} \quad \text{in } \Omega, \quad
T_i|_{\Gamma_{\text{top}}} = 0, \quad
\partial_n T_i + h T_i = 0 \text{ on } \Gamma_{\text{bottom}}, \quad
